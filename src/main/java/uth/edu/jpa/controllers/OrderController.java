...
public class OrderController {
    ...
    public String showOrderDetails(@PathVariable Long orderId, Model model, RedirectAttributes redirectAttributes) {
        // Lấy thông tin đơn hàng từ OrderService
        Optional<Order> order = orderService.findOrderById(orderId);

        if (order.isEmpty()) {
            redirectAttributes.addFlashAttribute("error", "Không tìm thấy đơn hàng.");
            return "redirect://player/order-history";
        }

        model.addAttribute("order", order);

        model.addAttribute("cartItems", order.get().getOrderItems());
        return "player/Player_Order-details";
    }
}
